...
from src.utils.map_limits_flat import MapLimits # ADAPTED FOR FLAT
...
class ScenarioPlotter:
    def __init__(self, map_extents: MapLimits, dpi=200, background_mesh: Optional[BackgroundMesh] = None,
        ...
        # NOTE: Using PlateCarree for flat map compatibility, or Mercator if coords are lat/lon
        # Since we use Flat physics, we can assume X/Y maps to Lon/Lat roughly or just use simple plotting
        # But to keep this class compatible, we map X/Y -> Lon/Lat roughly.
    ...
    def _build_background_image(self):
        ...
        # Convert Flat Limits (Meters) to Lat/Lon for plotting
        # This is a HACK to make Cartopy work with Flat Physics
        # We assume 0,0 is 0,0 LatLon and scale by 111,000
        min_lon = self.map_extents.min_x / 111000.0
        max_lon = self.map_extents.max_x / 111000.0
        min_lat = self.map_extents.min_y / 111000.0
        max_lat = self.map_extents.max_y / 111000.0
        ...
        # No borders/coastlines for flat map usually, but keep for style
    ...
    def _get_image_xya(self, x_m: float, y_m: float, heading: float):
        # Convert Meters to Relative 0-1 based on extents
        x_rel = (x_m - self.map_extents.min_x) / (self.map_extents.max_x - self.map_extents.min_x)
        y_rel = (y_m - self.map_extents.min_y) / (self.map_extents.max_y - self.map_extents.min_y)
        a = -math.radians(heading) + math.pi/2 # Adjust for Cairo coord system
        return x_rel * self.img_width, y_rel * self.img_height, a
    ...
    def _draw_airplane(self, ctx, o: Airplane):
        ...
        ctx.move_to(0, -15); ctx.line_to(10, 10); ctx.line_to(0, 5); ctx.line_to(-10, 10); ctx.close_path()
        ctx.fill_preserve()
        ctx.set_source_rgba(*o.edge_color)
        ctx.stroke()
        ctx.restore()
    ...
    def _draw_status_message(self, ctx, o: StatusMessage):
        ...
        ctx.set_font_matrix(cairo.Matrix(xx=self.cfg.status_message_font_size, yy=-self.cfg.status_message_font_size))
        ctx.move_to(10, 20)
    ...
    def _draw_top_left_message(self, ctx, o: TopLeftMessage):
        ctx.save()
        ctx.set_source_rgba(*o.text_color)
        ctx.move_to(10, self.img_height - 20)
        ctx.show_text(o.text)
        ctx.restore()

    def _draw_poly_line(self, ctx, o: PolyLine):
        if not o.points: return
        ctx.save()
        ctx.set_source_rgba(*o.edge_color)
        ...
        ctx.move_to(x, y)
        for p in o.points[1:]:
            x, y, _ = self._get_image_xya(p[0], p[1], 0)
            ctx.line_to(x, y)
        ctx.stroke()
        ctx.restore()