...
This module implements the PPO algorithm with several key features:
1. Sequence handling for recurrent policies (GRU)
2. Clipped surrogate objective for stable policy updates
3. Value function clipping for stable critic updates
4. Auxiliary world model for representation learning
5. KL early stopping to prevent catastrophic updates
6. NaN gradient guards for training stability
...
class PPOAgent:
    ...
    Key Methods:
    - update(): Perform PPO update on collected rollout data
    
    The agent handles:
    - Sequence-based training for recurrent policies
    - Graph batching for GNN critic
    - Actor freezing for critic warmup
    ...
    def __init__(self, model):
        ...
        Args:
            model: HybridActorCritic model instance.
        """
        self.cfg = Config
        self.model = model.to(self.cfg.DEVICE)

        # Optimization: Try to compile model with PyTorch 2.0.
        try:
        ...
        # Optimizer with weight decay to prevent parameter explosion.
    ...
    def update(self, obs, next_obs, actions, logprobs, returns, advantages, global_states=None, gru_states=None,
        ...
        This is the main PPO update function that processes a batch of
        collected rollout data and performs multiple epochs of updates.
        
        Args:
            obs: Observations [batch, obs_dim].
            next_obs: Next observations [batch, obs_dim] for world model.
            actions: Actions taken [batch, action_dim].
            logprobs: Log probabilities of actions [batch].
            returns: Computed returns [batch].
            advantages: Computed advantages [batch].
            global_states: List of PyG Data objects for GNN.
            gru_states: GRU hidden states [batch, 1, d_model].
            dones: Done flags [batch].
            old_values: Old value estimates [batch] for clipping.
            active_masks: Mask for valid timesteps [batch].
            update_actor: If False, freezes Actor updates (critic warmup).
            
        Returns:
            Dict with training statistics (loss, kl, clip_frac, etc.).
        """
        ...
        # Convert all inputs to tensors on device.
        b_obs = to_device(obs)
        ...
        # ==================== SEQUENCE HANDLING ====================
        # If using GRU, reshape flat buffers into (Batch, Seq_Len, Dim).
        # This preserves temporal structure for recurrent processing.
        if use_gru:
            ...
            # Reshape flat tensors to sequence format.
            def make_seq(x):
        ...
        for epoch_i in range(self.cfg.UPDATE_EPOCHS):
            ...
            for start in range(0, optim_batch_size, step_size):
                ...
                if use_gru:
                    ...
                    # Auxiliary world model forward pass.
                ...
                if update_actor:
                    ...
                    logratio = torch.clamp(logratio, -10, 10)  # Numerical stability.
                    ratio = logratio.exp()
                    ...
                    with torch.no_grad():
                        ...
                        # Skip batch if KL is too large (policy changed too much).
                        if approx_kl > 2.0:
                            continue
                ...
                # 2. Value Loss (with clipping for stability).
                ...
                # 3. Auxiliary Loss (World Model - predicts next state).
                aux_loss_elem = F.mse_loss(pred_next_state, target_next_state, reduction='none').mean(dim=-1)
                aux_loss_elem = torch.clamp(aux_loss_elem, 0, 10.0)  # Prevent explosion.
                aux_loss = (aux_loss_elem * mb_active).sum() / (mb_active.sum() + 1e-8)

                # TOTAL LOSS: Policy + Value + Auxiliary - Entropy.
                ...
                # NaN Check - skip update if gradients are invalid.
        ...
        # Safe mean to handle empty stats (all batches skipped).
        def safe_mean(k):
